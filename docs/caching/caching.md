...
- The `runway_cache_not_modified_total` Prometheus counter (with `route` label)
...
curl -s -X POST http://runway:8081/cache/purge \
  -H "Content-Type: application/json" \
  -d '{"all": true}' | jq .
```
...
curl -X POST http://runway:8081/cache/purge \
  -H "Content-Type: application/json" \
