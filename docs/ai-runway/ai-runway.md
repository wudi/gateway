...
When a client requests `"model": "cheap"`, the runway sends `"model": "gpt-4o-mini"` to the provider.
...
Error mapping:
| Provider Status | Runway Status | Error Type |
|----------------|---------------|------------|
...
When the client sends `"stream": true`, the runway:
