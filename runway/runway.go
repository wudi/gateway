...
import (
	...
	igw "github.com/wudi/runway/internal/runway"
)

// RunwayBuilder constructs a runway Server with custom features and middleware.
type RunwayBuilder struct {
	cfg              *Config
	configPath       string
	features         []Feature
...
// New creates a new RunwayBuilder for the given configuration.
func New(cfg *Config) *RunwayBuilder {
...
func (b *RunwayBuilder) WithConfigPath(path string) *RunwayBuilder {
	b.configPath = path
	return b
}

// WithDefaults registers all built-in features and middleware.
// This is what cmd/runway uses. Without this call, the runway starts
// with no built-in features — only custom ones you register.
func (b *RunwayBuilder) WithDefaults() *RunwayBuilder {
	b.useDefaults = true
	return b
}

// WithFeatures registers multiple custom features at once.
func (b *RunwayBuilder) WithFeatures(ff ...Feature) *RunwayBuilder {
...
func (b *RunwayBuilder) AddFeature(f Feature) *RunwayBuilder {
...
func (b *RunwayBuilder) AddMiddleware(slot MiddlewareSlot) *RunwayBuilder {
...
func (b *RunwayBuilder) AddGlobalMiddleware(slot GlobalMiddlewareSlot) *RunwayBuilder {
	b.globalSlots = append(b.globalSlots, slot)
	return b
}

// Build validates the configuration and constructs a ready-to-run Server.
func (b *RunwayBuilder) Build() (*Server, error) {
...
// Server wraps the internal runway server with a public API.
type Server struct {
	internal *igw.Server
}
